...
def generate_ctrl_pts(height, width, size=5, range=5):
    ...
    noise = np.random.uniform(-range, range, (size, size, 2)) # 2
    noise[[0,-1], :, :] = 0
    noise[:, [0,-1], :] = 0
...
if __name__ == "__main__":
    ...
    train_image = np.array(Image.open(sys.argv[1]).convert('L')).astype('float')
    train_label = np.array(Image.open(sys.argv[2]).convert('L')).astype('float')
    ...
    for j in range(gnum):
        imagename = 'image_'+str(j)+'.png'
        p, q = generate_ctrl_pts(height, width, size=size, range=rang) 
        # print p, q

        gimg = mls_rigid_deformation_inv(train_image, p, q)
        im = Image.new('L', (width, height))
        im.putdata(gimg.flatten().tolist())
        im.save(imagename)

        glbl = mls_rigid_deformation_inv(train_label, p, q)
        lb = Image.new('L', (width, height))
        lb.putdata(glbl.flatten().tolist())
        labelname = 'label_'+str(j)+'.png'
        lb.save(labelname)